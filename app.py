...
with customer_side:
    st.header('Customer')
    st.text_input('Customer ID:', value=st.session_state.menber_id)
    st.text_input('Customer Name:', value=st.session_state.menber_name)
    col1, col2 = st.columns([1,3])
    with col1:
        st.button('Identify')
    with col2:
        st.pills('Gender:', gender_choice, default=st.session_state.menber_gender)
    st.number_input('Age:', value=st.session_state.menber_age, step=1)
